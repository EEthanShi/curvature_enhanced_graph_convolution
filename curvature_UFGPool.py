...
def MyDataset(dataset, Lev, s, n, FrameType='Haar', add_feature=False, QM7=False):
    ...
    for i in range(len(dataset)):
        ...
        if args.CurvatureType == None:
            L = get_laplacian(data1.edge_index, num_nodes=num_nodes, normalization='sym')
        else:
        ...
        ########QM7 change G.edges() to G_orc.edges for the bug: KeyError: 0######
                A[n1, n2] = np.exp(-orc.G[n1][n2]["ricciCurvature"])
            #A[n1, n2] = orc.G[n1][n2]["ricciCurvature"]+min_rc
                
                #A[n1, n2] = 1/(1-(orc.G[n1][n2]["ricciCurvature"])) 
            #A[n1, n2] = abs(orc.G[n1][n2]["ricciCurvature"])  
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, default='PROTEINS',
                        help='name of dataset (default: PROTEINS), options: PROTEINS, Mutagenicity, DD, NCI1')
    parser.add_argument('--reps', type=int, default=4,  # should bbe 10, use 2 for parameter tunning 
                        help='number of repetitions (default: 10)')
    parser.add_argument('--epochs', type=int, default=200,
                        help='number of epochs to train (default: 200)')
    parser.add_argument('--patience', type=int, default=25,
                        help='early stopping criteria (default: 20)')
    parser.add_argument('--lr', type=float, default=5e-3,   # initially was 5e-3
                        help='learning rate (default: 5e-3)')
    parser.add_argument('--wd', type=float, default=1e-4, # previously 5e-4, 1e-4 qm7:41.07
                        help='weight decay (default: 5e-4)') #intitally was 5e-3
    parser.add_argument('--nhid', type=int, default=64, # previously was 64, for NCI1 we need at least 256 (Test accuracy: 0.79805), it seems higher nhid is better for the performance but slower 
...
#%%
    ...
    if dataname == 'qm7':
        ...
        #dataset = Data.from_dict(torch.load(path))
        dataset = TUDataset( path,'PROTEINS' )
    ...
    ResultCSV = args.dataset + 'Pooling_exp.csv'
#%%
    # training
    ...
    for r in range(num_reps):
        ...
        for epoch in range(epochs):
            ...
            #torch.set_default_dtype(torch.float32) #for all dataset except ogb ##########

            #torch.set_default_dtype(torch.float32)
            for i, data in enumerate(train_loader):
                #torch.set_default_tensor_type(torch.FloatTensor)
                #torch.set_default_dtype(torch.float64) 
                data = data.to(device,torch.float64) # for fixing bug, also can use torch.set_default_dtype(torch.float) or float 32: for dall datasets without qm7 and PROTEINS
                ### or to use data = data.to(device, torch.float) for all  other datasets. 
    ...
    print(np.average(np.load('results.npz')['saved_model_acc']))
    print(np.std([np.load('results.npz')['saved_model_acc']]))
